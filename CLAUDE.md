...
**Tarih (son guncelleme):** 4 Eylul 2026 ogleden sonra — **YARIN LISTESI 6/6 BITTI; sabahki 4 madde commit'te (`41d4742`), ogleden sonraki profil fotografi + gizlilik + acilis ekrani COMMIT DEGIL (Ayse onayi bekliyor); Ayse yeni Android dev build aliyor; store build ERTELENDI (test sonrasi tek build).** 4 Eyl'de eklenenler: telefon beyan usulu (kayitta zorunlu, SMS yok), Google/Apple ile giris + /profil-tamamla, TUREB rozeti (Edge Function `tureb-dogrula`, pg_net), TUREB dil listesi (39 + Turkce), yazim TUREB. 4 Eyl ogleden sonra: **profil fotografi + avatar (5) KODLANDI** (migration `profil_fotografi_avatar` canli, `lib/avatar.ts` + `hooks/use-avatarlar.ts` + `components/avatar.tsx`, expo-image-manipulator YENI NATIVE MODUL → yeni dev build; commit yok). **Gizlilik (6) da KODLANDI** (dm-gorseller ozel bucket, UI cumleleri, politika metinleri app+docs+store; DECISIONS #57). Acilis ekrani yeniden (duz kobalt + buyuyen pusula, `components/acilis-ekrani.tsx`). YARIN LISTESI 6/6 BITTI, commit yok. Detay STATE.md ust blok. **Onceki (3 Eyl):** BUYUK YENIDEN YAPILANMA + AJANDA/MASRAF PUSULASI. (1) **TAMAMEN UCRETSIZ MODEL:** RevenueCat/paywall/premium gate KALDIRILDI — `react-native-purchases` uninstall, `abone-ol.tsx`/`deneme-baslat.tsx`/`hooks/use-abonelik.ts`/`lib/revenuecat.ts` silindi, tum `premiumMi` dallari temizlendi (index/acil/bogaz/muzeler/sohbet/profil), kullanim kosullari + gizlilik metni guncellendi. Native modul gittigi icin **OTA YETMEZ, STORE BUILD SART** (sonraki surum 1.2.0). Magaza urunleri Ayse tarafindan satistan kaldirilacak; mevcut aboneler donem sonunda biter, iade yok. `yeni-kayit-hediye` Edge Function v3 deploy edildi (premium grant yok, sadece 'tamamen ucretsiz' hos geldin maili; kaynak `supabase/functions/yeni-kayit-hediye/index.ts`). Supabase MCP ARTIK Pusula projesini goruyor (3 Eyl, baglayici yeniden baglandi). Magaza urunleri satistan kaldirildi. (2) **ADMIN PANELI KALDIRILDI → INLINE YONETIM:** `app/admin*.tsx` (9 dosya) silindi, `components/yetkili/` altinda `YetkiliBolum` sarmalayicisi (yalnizca admin/moderator gorur, 'YONET ·' satiri, acilir panel) + 7 yonetim bileseni: SahaYonetim (ana sayfa, CanliDurumOzet alti), EtkinlikYonetim (EtkinliklerBandi alti), SohbetYonetim (sohbet FlatList ListHeaderComponent, 3 sekme: Raporlar/Banlar/Kufur, admin), MekanSaatleriYonetim (muzeler sekmesi, aktif kategoriyle senkron), UlasimTarifeYonetim tip='bogaz'|'havalimani' (bogaz/ulasim sekmeleri, admin), AcilRehberYonetim (acil, admin), ModeratorYonetim (profil, admin). Is mantigi/Supabase sorgulari birebir korundu. tsc temiz (13 Deno hatasi bilinen). (3) **Maliyet raporu:** `claude-context/raporlar/maliyet-raporu-2026-09.md` (~82-89 $/ay). **KIRMIZI CIZGI: Resend/Expo/Supabase abonelikleri Hey Istanbul ile ORTAK — asla iptal/downgrade onerme.** (4) **UI REDESIGN KODLANDI (3 Eyl gece): 'Kobalt & Menekse'** — `constants/theme.ts` v2 + `components/ui/pusula-ui.tsx` ortak parcalar, 31 dosya yeniden boyandi, fonksiyon kaybi sifir (envanter karsilastirmali), tsc app kodu 0 hata. Kurallar PROJECT.md Bolum 5. Cihaz testi + commit + 1.2.0 store build bekliyor. (5) **SOHBET TEPKI + YANIT** eklendi (migration `sohbet_tepki_ve_yanit`, push-gonder v5 — premium push filtresi de kalkti). Detay PROJECT.md Bolum 9. (6) **SOHBETTE GORSEL** (migration `sohbet_gorsel_paylasimi`, bucket sohbet-gorseller). (7) **BOGAZ: hafta ici + hafta sonu ayni anda.** (8) **"REHBER ARANIYOR" ILAN sekmesi** (Ara'nin yerine; push-gonder v6 dil filtreli, 7. bildirim kategorisi, takvim + TUREB taban kontrolu, 40 dil (TUREB listesi 39 + Turkce en sonda, 4 Eyl)) + ~~ROTA PLANLAYICI~~ (ikinci oturumda SILINDI). (10) **AJANDA + MASRAF PUSULASI (3 Eyl ikinci oturum):** migration `ajanda_ve_masraf_pusulasi`, Edge Function `masraf-disa-aktar` (PDF/Word/Excel logolu, palet; pdf-lib/docx/exceljs, deploy edildi + E2E test basarili), `app/ajanda.tsx`, `app/tur/[id].tsx`, ana sayfa `AjandaKarti`; gonderim telefonun mail uygulamasi ekli (Ayse karari). expo-mail-composer/sharing/file-system eklendi (native). tsconfig `exclude: supabase/functions` → tsc 0 hata. Detay PROJECT.md 9c, DECISIONS #53, STATE.md ust blok. (9) **OZEL MESAJLASMA (DM)** — migration `ozel_mesajlasma`, `hooks/use-dm.ts`, `app/dm/[id].tsx`, sohbette "Genel | Mesajlarim"; admin okuyamaz. Detay PROJECT.md 9a/9b/9c, DECISIONS #52. **YENI OTURUM: STATE.md en ustteki "YENI OTURUM BURADAN BASLA" blogunu oku — YARIN ILK IS listesi orada (telefonla uyelik + SMS dogrulama, Google/Apple giris, TUREB ruhsat kontrolu, ilan dil listesi guncelleme, profil fotografi + sohbette avatar, GIZLILIK: DM + ajanda/masraf admin dahil kimseye gorunmez — RLS zaten oyle, DM gorselleri ozel bucket'a + UI/politika metinleri); ayrica sira: telefonda test (YENI DEV BUILD sart: mail-composer/sharing native) → commit (Ayse onayiyla) + push (docs/varliklar yayina cikar) → app.json 1.2.0 → EAS store build (OTA yetmez).** **Onceki (8 Tem):** **YEREBATAN SARNICI MUZEKART DEGISIKLIGI (sadece DB, kod degismedi).** Yerebatan'in yonetimi Kultur AS'den **Vakiflar Genel Mudurlugu'ne gecti** (6 Haz 2026 yeniden acilis; Haziran boyunca TC vatandasina ucretsizdi). **1 Tem 2026 itibariyla TC vatandaslari icin MuzeKart GECERLI** (Museum Pass gecmez). `mekan_saatleri` (id `70f1921b...`) guncellendi: `muzekart='gecerli'` + `muzekart_not='Sadece T.C. vatandaşı, 1 Tem 2026 itibarıyla; Museum Pass geçmez'` + `ozel_not` (emoji temizlendi, Vakiflar devri notu eklendi). Kod degisikligi GEREKMEDI — MuzeKart ekrani (gecen/gecmeyen listeleri), arama etiketi ve muzeler detay modali hepsi DB `muzekart` alanindan beslenir, realtime ile aninda yansidi. **UYARI: yerebatan.com (Kultur AS) artik BAYAT kaynak** — hala "Muze Kart gecmez" + "gecici kapali" yaziyor, teyit icin KULLANMA. BEKLEYEN: (a) Vakiflar donemi guncel giris fiyatlari (DB'de hala Kultur AS donemi 450/2250 TL — fiyat degisikligi push tetikler, bilerek dokunulmadi), (b) `kaynak`/`site` alanlari hala yerebatan.com'u gosteriyor, yeni resmi kaynak bulununca guncelle. NOT: Supabase MCP hala SADECE hey-istanbul projesini goruyor (bkz. STATE.md 6 Tem notu); islem `.env` SUPABASE_SERVICE_ROLE_KEY ile REST uzerinden yapildi. **Onceki (2 Tem):** **BILDIRIM TERCIHI BUG'I COZULDU (duplike push token, ISSUES #87 + DECISIONS #51).** Trafik kapatilmasina ragmen push geliyordu; tercih sistemi saglamdi — S22'nin token'i Ela'nin profilinde DE kayitliydi (26 Haz moderator testi girisi, tercih NULL=hepsi acik) + logout token temizligi SIGNED_OUT'ta (signOut SONRASI) calistigi icin RLS sessiz reddediyordu. Fix: Ela token NULL (aninda kesildi) + `push_token_kaydet` SECURITY DEFINER RPC (token kaydinda ayni token'i diger profillerden supurur; migration `push_token_kaydet_rpc_token_benzersizligi`) + use-push-token.ts RPC'ye gecti + `pushTokenTemizle()` signOut ONCESI (profil.tsx 2 nokta) + push-gonder Edge Function v4 token dedupe. 51 mimari karar. **DAGITILDI:** commit `599d86d` + EAS Update OTA group `32247996-f854-4ab8-884f-68d2c60fab66` (runtime 1.1.1, android+ios). Kural: Expo push token CIHAZA aittir hesaba degil; signOut sonrasi RLS'li UPDATE olu koddur. **Onceki (26 Haz):** **MODERATOR CAMI YONETIMI ACILDI (OTA).** Moderatorler Mekan Saatleri'nde artik TUM camileri duzenler + yeni cami ekler (`app/admin-saatler.tsx`: kategori moderatorde `camiler`e kilitli, liste/ekle/duzenle `isYetkili`; mevsim gecisi + kategori sekmeleri + mekan SILME admin'e ozel; RLS zaten admin+moderator'a aciydi, degisiklik tamamen frontend). Genel duyurular da zaten moderatorlerde aciktı (ana sayfa GenelDuyuruPanel). EAS Update OTA group `198c7265-9a4f-4253-8cf0-e8bb463d937a`, runtime 1.1.1, commit `53fdf07`, android+ios. Bkz. PROJECT.md Bolum 8 (Moderator Yetkileri) + STATE.md. **Onceki (16 Haz):** v1.1.1 HER IKI PLATFORMDA YAYINDA. 49 mimari karar. Bu oturumda (14-16 Haz): **Acil ekrani UI bug fix.** Admin panelden acil kismina eklenen "TURYOL BOGAZ HATTI" numarasi (kategori `acil_numara`, aktif=true, DB'de sorunsuz) app'ta gorunmuyordu. Kok sebep: `app/(tabs)/acil.tsx`'te "Acil Durum" bolumu SABIT KODLANMIS sadece 112 kartini basiyordu; hook `acilNumaralar` listesini cekip `aciller` degiskenine atiyor ama JSX'te HIC render etmiyordu (2021 acil numara birlesimi sonrasi sadelestirme kalintisi). Yani `acil_numara` kategorisine ne eklenirse eklensin gorunmez. **Fix:** 112 kartinin altina "Faydali Telefonlar" bolumu eklendi — `acil_numara` kategorisindeki 112 disindaki kayitlari (`numara !== '112'` filtresi) meslek_kurulusu pattern'iyle (isim + "Ara" butonu + opsiyonel Web) liste render eder. Bundan boyle bu kategoriye eklenen her dogrudan-arama numarasi otomatik gorunur, realtime listener ile aninda. **Dagitim: EAS Update OTA** (runtime 1.1.1, update group `6929e072-9373-4708-a926-e2287eff9ea2`, android+ios), magaza build'i GEREKMEDI. Commit `cda8fac`. Test: Ayse cihazinda TURYOL karti dogrulandi. **AYRICA PUSH UCTAN UCA TEST + SES FIX:** iOS push tam (banner+ses); Android S22'de bildirim geldi ama SES YOKTU. Kok sebep: -v2 kanallari `sound` verilmeden olusturuldugu icin Samsung OneUI'da SESSIZ (expo-notifications 0.32.16 kaynagi: `sound` anahtari yoksa `setSound` cagrilmaz — #45'in "omit=sistem sesi" teshisi YANLISTI). Fix (v1.1.2): kanallar `sound: 'default'` (-> DEFAULT_NOTIFICATION_URI) + ID'ler `-v3`, eski v1/v2 silinir; Edge Function `push-gonder` v3 (`KANAL_MAP` -v3) → OTA almamis cihazlarda default kanal fallback → ses ANINDA geri geldi (65 kullanici). Dagitim: function deploy v3 + EAS Update OTA group `ff7eceb9`. Trigger zinciri saglikli dogrulandi (10 fonksiyon SECURITY DEFINER, net._http_response son 25 cagri 200). Bkz. DECISIONS #49 + ISSUES #86. **AYRICA (oturum devami):** (a) iOS sohbet klavye fix dagitildi (ISSUES #85, FlatList keyboardDismissMode+persistTaps, OTA group `7b10c8d6`). (b) `sehir-hatlari-iptal-takip` SESSIZ MODA alindi — delta=0 ise rapor/bildirim yok + notifyOnCompletion kapatildi → 964 okunmamis bildirim birikimi durdu. (c) Leaked Password Protection Dashboard'tan ACILDI (advisor'da uyari kalkti, teyit edildi). (d) **KVKK HESAP SILME — Ayfer Artuç** (id `0bbcab41...`, afa_ss@hotmail.com): uclu dogrulama (id+email+isim) + iliskili tablolarda 0 kayit → atomik profiles+auth.users DELETE (id+email guard, RETURNING) + KVKK Madde 11 kapanis maili (Resend `24b3352b`). Abonelik aktif gorunuyordu ama 7-gun trial'iydi (gercek RC satin almasi degil, iade gerekmedi). Atakan'a ozel script GENERIC `scripts/hesap-silme-onay.mjs`'e cevrildi — bundan boyle HEDEF blogu duzenlenip --dry/--send. (e) OPSIYONEL kalan: advisor'da 19 "SECURITY DEFINER fonksiyon RPC'den cagrilabilir" uyarisi (trg_push_* EXECUTE revoke ile temizlenebilir, push kirilmaz — Ayse onayi bekliyor). Onceki oturum (5-6 Haz): (1) **KRITIK: Push notification'lar 2 Haz'dan beri sessiz oluydu, duzeltildi** — Security Advisor temizliginde `push_gonder_async`'in PUBLIC EXECUTE'u revoke edilince trigger'lar (SECURITY INVOKER) authenticated kullanicida "permission denied" aliyordu; zirhsiz 3 trigger'da (bogaz/havalimani/mekan_saatleri) admin panel UPDATE'leri komple FAIL, zirhli 6'sinda push sessizce olu. Fix: 9 `trg_push_*` fonksiyonu SECURITY DEFINER + 3'une EXCEPTION sargisi (migration `push_trigger_security_definer_ve_exception_zirh`). DECISIONS #48 + ISSUES #84. **Push uctan uca testi HALA BEKLIYOR** (sohbet mesaji → kapali cihaza push). (2) **Bogaz turu tarife otomasyonu kuruldu:** `turyol-senkron` scheduled task (07:30+19:30, `scripts/turyol-senkron.mjs`, turyol.com form POST — SCRIPTS.md #3) + `bogaz-diger-senkron` (08:00+20:00, Sehir Hatlari kisa/uzun + Dentur, prompt-driven Firecrawl cunku Dentur SPA — INFRASTRUCTURE Bolum 11). TURYOL tarife elle duzeltildi (haftaici saat basi 10-21, hafta sonu 14 sefer), Sehir Hatlari'nin BOS duran saat alanlari dolduruldu (kisa 14:40, uzun 10:35). (3) Leaked Password Protection Dashboard'ta acilacakti (Attack Protection → "Configure in email provider"), onay teyit edilmedi — kontrol et. (4) `sehir-hatlari-iptal-takip` 964 okunmamis bildirim biriktirmis — prompt'u sessiz moda cevrilmeli (bekleyen is). Onceki ozet (4 Haziran): v1.1.1 iOS onayi geldi, app_versions iOS 1.1.1 UPDATE edildi. Genel Duyuru silme bug'i cozuldu: butonlar tum paketlerde VARDI ama `kart` stili `flexDirection: 'row'` kaldigi icin 0 genislikte gorunmez render oluyordu; fix kart → column+stretch (commit 29ee7a6), dagitim EAS Update OTA ile (runtime 1.1.1, magaza build'i GEREKMEDI — OTA'nin ilk basarili gercek kullanimi). Tani tuzaklari (Hermes bundle'da Turkce karakterli string grep yanlis negatifi): DECISIONS #47 + ISSUES #83. Bir sonraki store surumu 1.1.2 olacak. Daha onceki ozet (2 Haziran gece): v1.1.0 Android sabah yayına çıktıktan sonra ayni gün öğleden sonra iki kritik bug fark edildi (Android push notification ses gelmiyor + Genel Duyuru silme UI eksik) → hemen v1.1.1 hotfix paketlendi. iOS v1.1.0 review iptal edildi, doğrudan v1.1.1'e geçildi. Android v1.1.1 onaylandı, Ayşe öğleden sonra "Yayınla" bastı, app_versions UPDATE ile eski sürüm kullanıcılarına güncelleme bandı görünüyor. iOS Apple Review devam ediyor (~24-72 saat). **Bu oturumda (2 Haziran, sabah → gece) BÜYÜK İŞLER:** (1) **Havaist Senkron Pipeline kuruldu** — hava.ist resmi backend API'si keşfedildi (`s.hava.ist/api.php`), `scripts/havaist-senkron.mjs` + scheduled task `havaist-senkron` (cron `0 7 * * *`), 14 IST kaydı taze veriyle + hat kodları (HVL-1..9, HVİST-5A/7/11/13) + her iki yönde güzergah text alanları (DECISIONS #44, SCRIPTS.md #2). (2) **Push notification ses bug fix** — `setNotificationChannelAsync({ sound: 'default' })` string'i sessiz kanal oluşturuyordu, server-side Edge Function push-gonder v2 deploy (KANAL_MAP `-v2` suffix) ile mevcut v1.1.0 kullanıcısı için anlık çözüldü, client-side fix v1.1.1'de (eski kanallar silinir, yeni `-v2` ID'li sound parametresiz oluşur) (DECISIONS #45). (3) **Yeni kayıt 7 gün premium trial sistemi** — Vault'a `resend_api_key`, Edge Function `yeni-kayit-hediye`, SQL trigger `trg_yeni_kayit_hediye` ON profiles INSERT, markalı hoş geldin maili (Ayşe onaylı) — anlık tetikleme, idempotent (DECISIONS #46). (4) **Genel duyuru silme defansif kod** — RLS sessiz reddi yakalama (DELETE ... RETURNING), optimistic state update, Alert ile başarısızlık bildirimi (v1.1.1'de aktif). (5) **GitHub'a 1 aylık birikmiş 58 dosya yedeklendi** — kritik untracked durumdaydı, commit + push yapıldı. (6) **Security Advisor temizliği** — 32 warning → 3 (12 search_path fix + 8 PUBLIC EXECUTE revoke + storage list policy + rls_auto_enable), kalan 3 false positive (is_admin/is_admin_or_mod RLS için zorunlu) + Leaked Password Protection (Ayşe Dashboard'tan açacak). **46 mimari karar** (#44 Havaist API, #45 Android kanal ses bug, #46 Yeni kayıt trial).

> **DIS BAGIMLILIK UYARISI (4 Tem 2026, Ayse onayli):** `havalimani_seferleri` tablosunu **Hey Istanbul** uygulamasi da SALT OKUNUR okur (anon SELECT, `/Users/aysetokkus/hey-istanbul/lib/pusula-veri.ts` — turist yuzunde canli Havaist/Havabus tarifesi). Bu tablonun SEMASINI degistirmeden veya RLS anon SELECT iznini kaldirmadan once Hey Istanbul tarafini kontrol et; `havaist-senkron` scheduled task'i iki urunu birden besliyor.
...
| Google ile giris, Apple ile giris, OAuth, signInWithOAuth, expo-web-browser, profil-tamamla, profilEksik, Google Cloud consent/branding, client secret invalid_client | `PROJECT.md` (6a) + `DECISIONS.md` (#55) + `ISSUES.md` (#89) + `lib/oauth.ts` + `app/profil-tamamla.tsx` |
| Telefon, telefon dogrulama, SMS, OTP, WhatsApp kodu, telefon zorunlu, TelefonModal, TelefonKarti, numara bildir | `PROJECT.md` (9b-1) + `DECISIONS.md` (#54) + `lib/telefon.ts` + `components/telefon-modal.tsx` |
| TUREB, ruhsat kontrolu, rehber veritabani, tureb-dogrula, tureb_durum, eylemli/eylemsiz, rozet, coklu eslesme, pg_net TLS | `PROJECT.md` (9b-2) + `DECISIONS.md` (#56) + `ISSUES.md` (#88, #90) + `hooks/use-tureb.ts` + `supabase/functions/tureb-dogrula/` |
| Profil fotografi, avatar, avatar_url, profil-fotolari bucket, expo-image-manipulator, harf avatari yerine resim | `PROJECT.md` (9b-3) + `lib/avatar.ts` + `hooks/use-avatarlar.ts` + `components/avatar.tsx` |
| Gizlilik, KVKK, ozel mesaj gizliligi, dm-gorseller, imzali URL, masraf gizliligi, politika metni, docs/index.html yasal | `DECISIONS.md` (#57) + `app/gizlilik-politikasi.tsx` + `docs/index.html` (#yasal) |
| Acilis ekrani, splash, squircle/yuvarlak icinde logo, Android 12 splash maskesi, AcilisEkrani | `components/acilis-ekrani.tsx` + `STATE.md` (4 Eyl ACILIS EKRANI blogu) — kural: acik/koyu zemin AYNI renk, native gorsel yazisiz |
| Bildirimler ekrani, bildirim gecmisi, bildirim_gecmisi, zil, okunmamis, bildirime dokununca yonlendirme, push-gonder v7 | `PROJECT.md` (9d) + `lib/bildirim-yonlendir.ts` + `hooks/use-bildirim-gecmisi.ts` + `app/bildirimler.tsx` |
| Klavye formu kapatiyor, KeyboardAvoidingView, ModalKapak klavye, edge-to-edge klavye | `components/ui/pusula-ui.tsx` (ModalKapak icinde padding) + `STATE.md` (4 Eyl aksam rotuslari #3) |
| Form ici aciklama, placeholder, alt yazi, gizlilik cumlesi uslubu (sen/siz) | `DECISIONS.md` (#57 sonu — Ayse kurallari) |
| Dil listesi, diller.ts, TUREB dilleri, Turkce secenegi | `constants/diller.ts` (39 TUREB dili + Turkce en sonda; kaynak xls Ayse'den, 4 Eyl 2026) |
...
| Android push sessiz, ses gelmiyor, notification channel sound default bug, kanal -v2/-v3 suffix, Samsung OneUI sessiz kanal | `DECISIONS.md` (#49 DOGRU COZUM `sound: 'default'` + -v3 — #45'i duzeltir; #45 teshisi yanlisti) + `ISSUES.md` (#86). Kural: `sound` omit etme, acikca `sound: 'default'` ver |
...
| Bildirim tercihi calismiyor, kapattigim bildirim geliyor, bildirim_tercihleri, duplike push token, expo_push_token iki profilde, push_token_kaydet RPC, pushTokenTemizle, logout token temizligi | `DECISIONS.md` (#51 "Expo Push Token CIHAZA aittir") + `ISSUES.md` (#87). Kural: token kaydi RPC ile (diger profillerden supurur), temizlik signOut ONCESI |
| Acil ekrani, acil_numara, Faydali Telefonlar, admin panel telefon eklendi gorunmuyor, use-acil-rehber, TURYOL numara | `app/(tabs)/acil.tsx` — 112 karti sabit; diger `acil_numara` kayitlari "Faydali Telefonlar" bolumunde render olur (14 Haz 2026 fix, commit `cda8fac`) |
| Hesap silme, KVKK Madde 11, veri imha, hesabimi silin, GDPR, kullanici silme talebi, kapanis maili | `scripts/hesap-silme-onay.mjs` (GENERIC — HEDEF blogu duzenle, --dry/--test/--send). Adimlar: uclu dogrulama (id+email+isim) + iliskili tablo kontrolu → atomik `profiles`+`auth.users` DELETE (id+email guard, RETURNING) → KVKK kapanis maili. Ornek: Atakan (27 May), Ayfer Artuç (16 Haz 2026, STATE.md) |
...
| Is ilani, ilanlar, rehber araniyor, yedek rehber, dil filtresi, profiles.diller, ilanlar-v3 | `PROJECT.md` (9b) + `app/(tabs)/ilanlar.tsx` + `hooks/use-ilanlar.ts` |
| Ajanda, tur takvimi, Ajandam karti, masraf pusulasi, masraf, avans, fis, PDF/Word/Excel disa aktarma, acenteye mail, masraf-disa-aktar, ajanda_turlar, masraflar, masraf-fisler | `PROJECT.md` (9c) + `DECISIONS.md` (#53) + `hooks/use-ajanda.ts` + `hooks/use-masraflar.ts` + `app/tur/[id].tsx` + `supabase/functions/masraf-disa-aktar/` |
| Rota planlayici, rota-motoru, rotalar tablosu (ESKI — 3 Eyl 2026'da SILINDI) | `DECISIONS.md` (#53) — kod git gecmisinde (commit `aa87bfe` sonrasi calisma kopyasinda hic commit edilmedi; ihtiyac olursa yeniden yazilir) |
| Sohbet tepki, begen/kalp/saskin, yanit, yanit_id, sohbet_tepkileri, MesajMenusu, cift dokunma, hedefli push, haric_liste | `PROJECT.md` (Bolum 9) + `hooks/use-sohbet-tepkileri.ts` + `components/sohbet-tepkiler.tsx` |
...
| Tasarim kurali, emoji, renk, logo, font, kicker, Kart, BirincilButon, ModalKapak, theme.ts, pusula-ui | `PROJECT.md` (Bolum 5 "Tasarim Sistemi v2 — Kobalt & Menekse") + `components/ui/pusula-ui.tsx` |
