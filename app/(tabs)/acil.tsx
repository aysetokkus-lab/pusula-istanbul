...
const FALLBACK_ACIL = [
  { isim: 'Ambulans', numara: '112' },
  { isim: 'İtfaiye', numara: '110' },
  { isim: 'Polis', numara: '155' },
  { isim: 'Jandarma', numara: '156' },
  { isim: 'Deniz Kurtarma', numara: '158' },
];
...
export default function Acil() {
  ...
  return (
    <ScrollView style={s.container}>
      ...
      {/* TURIZM POLISI */}
      <View style={s.bolum}>
        <Text style={s.bolumBaslik}>Turizm Polisi</Text>
        <TouchableOpacity
          style={s.turizmKart}
        ...
        >
          <View style={s.turizmSol}>
            <Text style={s.turizmIsim}>{tp.isim}</Text>
  {/* aciklama kaldirildi */}
          </View>
      ...
      {/* ACIL NUMARALAR */}
      <View style={s.bolum}>
        <Text style={s.bolumBaslik}>Acil Numaralar</Text>
        <View style={s.acilGrid}>
          {aciller.map((a: any, i: number) => (
            <TouchableOpacity
              key={a.id || i}
              style={s.acilKart}
              onPress={() => ara(a.numara || '')}
              activeOpacity={0.7}
            >
              <Text style={s.acilNumara}>{a.goruntu || a.numara}</Text>
              <Text style={s.acilIsim}>{a.isim}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      {/* MESLEK KURULUSLARI */}
      <View style={s.bolum}>
        <Text style={s.bolumBaslik}>Meslek Kuruluşları</Text>
        {meslekler.map((k: any, i: number) => (
          <TouchableOpacity
            key={k.id || i}
            style={s.listeSatir}
            onPress={() => ara(k.numara || '')}
            activeOpacity={0.7}
          >
            <View style={s.listeBilgi}>
              <Text style={s.listeIsim}>{k.isim}</Text>
              <Text style={s.listeNumara}>{k.goruntu || k.numara}</Text>
            </View>
            <View style={s.araBtn}>
              <Text style={s.araBtnYazi}>Ara</Text>
            </View>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {/* FAYDALI KAYNAKLAR */}
      <View style={s.bolum}>
        <Text style={s.bolumBaslik}>Faydalı Kaynaklar</Text>
        {linkler.map((l: any, i: number) => (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text style={s.listeIsim}>{l.isim}</Text>
...
const s = StyleSheet.create({
  ...
  bolumBaslik: { color: '#8A9BAE', fontSize: 13, fontWeight: '600', marginBottom: 12 },
  ...
  acilIsim: { color: '#64748B', fontSize: 11, textAlign: 'center', fontWeight: '500' },
  ...
  turizmIsim: { color: '#1E293B', fontSize: 15, fontWeight: '700' },
  turizmAciklama: { color: '#64748B', fontSize: 12, marginTop: 3 },
  ...
  listeBilgi: { flex: 1 },
  listeIsim: { color: '#1E293B', fontSize: 14, fontWeight: '600', flex: 1 },
