...
> ## YENI OTURUM BURADAN BASLA (4 Eylul 2026, ogleden sonra — ikinci oturum kapanisi)
>
> **COMMIT `2ff498f` (4 Eyl 2026 aksam, Ayse: "commit yap")** — bugunku 49 dosya tek commit'te (profil fotografi, gizlilik, acilis ekrani, uygulama ici bildirimler, rotuslar, docs). **PUSH VM'den olmadi (SSH yok) → Ayse Terminal'de `git push origin main`.** Push sonrasi docs/index.html (yeni gizlilik politikasi + kosullar) GitHub Pages'te canli olur. `aboneler-premium.xlsx` bilerek commit disi. **Apple Developer capability + Supabase Apple provider Ayse tarafindan YAPILDI (4 Eyl aksam teyit).** Sirada: push (Ayse) → TEK store build 1.2.0.
>
> **OTURUM KAPANISI (4 Eyl 2026 ~14:00, Ayse: "md guncelle").** Bu oturumda: **madde 5 profil fotografi + avatar**, **madde 6 gizlilik**, **acilis ekrani yeniden** — hepsi KODLANDI, Supabase tarafi CANLI (migration `profil_fotografi_avatar`, `dm_gorselleri_ozel_bucket`), **COMMIT YOK** (`git status` ~30 dosya; `.git/index.lock` kalintisi olabilir → silme izni). tsc 0; eslint yalnizca `kullanim-kosullari.tsx`'te 7 ESKI tirnak hatasi. **YARIN LISTESI 6/6 BITTI.** Ayse cihazda dogruladi: profil fotografi calisiyor, buyuyen pusula "sahane". **Ayse yeni Android dev build aliyor** (`eas build --platform android --profile development`; gerekce: expo-image-manipulator native + splash native). **Yeni oturumda sira:** (1) yeni build'de cihaz testi: splash (kare + kesik yazi gitmeli), foto kucultme (512px), DM gorseli (ozel bucket + imzali URL), politika sayfalari; (2) Ayse "commit'le" derse TEK commit + push (onerilen mesaj: `Profil fotografi + avatar, gizlilik (dm-gorseller, politika metinleri), acilis ekrani yeniden` + Co-Authored-By) → docs/index.html yayina cikar (yeni gizlilik politikasi magaza linkinde gorunur); (3) Ayse: Apple Developer capability + Supabase Apple provider, Google marka dogrulamasi e-postasi, Resend key yenileme (sabah notu); (4) docs/index.html eski mavi palet (ayri is); (5) app.json build numaralari (calisma kopyasi 44/45) → TEK store build 1.2.0 (`v1.2.0-yayin-komutlari.md`). **Politika metni kurali (Ayse):** "veritabani sahibinin teknik erisimi var / rutin kullanilmaz" cumlesi CIKARILDI, yeniden ekleme; yalnizca "uygulama uzerinden hicbir yonetici goremez, RLS, aktarimda ve depoda sifreli". Detay bloklari asagida (tarih etiketli).
>
> **[4 Eyl 2026 aksam] OGLEDEN SONRA ROTUSLARI (Ayse cihaz testi sonrasi, hepsi kodlandi, commit yok):** (1) Ana sayfa: selamlama 22px + adjustsFontSizeToFit; **header marka blogu splash ile ayni** (pusula ortada dairesiz + PUSULA / ISTANBUL alt alta, arama sagda mutlak, zil solda). (2) Rehber Araniyor: baslik alti aciklama, Ilan Ver alt yazisi ve form notlari silindi. (3) **KLAVYE FIX — `ModalKapak` icinde `KeyboardAvoidingView behavior="padding"` (iki platform)**; ekranlardaki dis sarmalayicilar `behavior={undefined}` (notr). 'position' denendi, ust alanlar tasti → padding. Yetkili panellerinin kendi modallari (mekan saatleri/tarife/acil/etkinlik/sohbet yonetimi) KAPSAM DISI. (4) **FORM ICI ACIKLAMA YOK** (tum formlar: alt yazilar, gri notlar, 'Orn.' placeholder'lar → duz etiket; hata/sayac/durum satirlari kaldi). (5) Gizlilik cumleleri Ayse onayli metinlerle yeniden ("siz" dili; DECISIONS #57). (6) Politika: "ruhsatnamenizde ekli olan diller". (7) **SORUMLULUK SINIRI:** `BilgiNotu` bileseni (pusula-ui) muzeler/bogaz/ulasim/muzeKart ekranlarinin altinda; Kullanim Kosullari madde 7 yeniden + madde 9 ek (app + docs/index.html). (8) **Mekan saatleri yonetim formuna** gece muzeciligi (gece_acilis/kapanis/gise), ulasim_notu, ekstra alanlari eklendi (Yerebatan gece muzesi silinemiyordu — DB'de 18:30–22:00 duruyor, Ayse formdan bosaltacak). (9) **UYGULAMA ICI BILDIRIMLER:** migration `bildirim_gecmisi` (RLS: hedef=ben ya da haric olmayan herkes; pg_cron `bildirim-gecmisi-temizlik` 03:15, 30 gun), **push-gonder v7** (her gonderimde satir; DM'de icerik "Özel mesaj"), `lib/bildirim-yonlendir.ts` (kategori+veri → rota), `hooks/use-bildirim-gecmisi.ts` (liste + okunmamis; tercih/dil filtresi push ile birebir; AsyncStorage `bildirim-son-goruldu`), `app/bildirimler.tsx` (Stack; gune gore gruplu; yeni satirlar accent'li), ana sayfa header solda zil + kirmizi nokta, `hooks/use-bildirimler.ts` dokunma yonlendirmesi (sicak + soguk baslangic `getLastNotificationResponseAsync`, 2 dk sinir, `useBildirimler(hazir)` fontsLoaded'dan SONRA cagrilir). E2E: Ayse'ye hedefli test push → satir olustu. tsc 0.
>
> **[4 Eyl 2026 gece] v1.2.0 STORE BUILD + SUBMIT.** Android build 4f91d5a8 (versionCode 46) OK → `eas submit --platform android --latest` (Play Console DRAFT). iOS ilk build a623e333 (build 45) BASARISIZ: non-interactive'de provisioning profile yenilenemedi (Sign in with Apple capability yeni). Interaktif build: Apple ID **aysetokkus@hotmail.com** (= Apple Developer hesabi; gmail DEGIL — INFRASTRUCTURE duzeltildi, eas.json appleId hotmail), ilk denemede -20209 "locked" (Keychain'deki eski sifre) → sifre/keychain yenilendi → 2FA → eski profil J3L4CH5G5Q "no longer valid" → Reuse: n, Generate: Y → build 46 OK → `eas submit --platform ios --latest` → App Store Connect'e yuklendi (submission ed28ec67; Apple isleme 5-10 dk, TestFlight'ta build 46). **SIRADA:** TestFlight testi (Apple ile giris ilk kez gercek cihazda), App Store Connect 1.2.0 surumu + build 46 + ucretsiz aciklama + review notu + Manual Release; Play Console taslak release + magaza aciklamasi + rollout. Adimlar `v1.2.0-yayin-komutlari.md` 4-5. Yayin sonrasi: app_versions UPDATE, duyuru + push, CHANGELOG "yayinda".
>
> ### Onceki blok (4 Eylul 2026, ogle — ilk oturum kapanisi)
>
> **OTURUM KAPANISI (4 Eyl 2026 ~12:00, Ayse: "tum md'leri guncelle, 5 ve 6 icin yeni oturum acacagim").** Her sey COMMIT + PUSH edildi: `d3fa5f9` (dunku 97 dosya), `b03fe09` (surum 1.2.0), **`41d4742`** (bugunku 4 madde: telefon beyan usulu, Google/Apple giris, TUREB rozeti, TUREB dil listesi). `git status` temiz (yalnizca `aboneler-premium.xlsx` degisik — bilerek commit disi). Supabase tarafi CANLI (migration'lar + Edge Function `tureb-dogrula` v3). **Yeni oturumda sira:** (1) ~~5. madde profil fotografi~~ **YAPILDI (ogleden sonra blogu asagida, commit yok)** — eski not: **5. madde: profil fotografi + sohbette avatar** (varsayimlar: istege bagli; kare kirp + 512px kucult; bucket `profil-fotolari` herkes okur / sahibi yazar; `profiles.avatar_url`; sohbet/DM balonu, ilan karti, DM listesi, tepki verenler listesinde harf avatari yerine resim, foto yoksa harf kalir; expo-image-picker zaten var). (2) **6. madde: gizlilik** — DM gorselleri `sohbet-gorseller` (public) → DM icin OZEL bucket + imzali URL; DM ekrani / ajanda-tur ekrani / profilde acik gizlilik cumleleri; `app/gizlilik-politikasi.tsx` + `kullanim-kosullari.tsx` + magaza metnine "ozel mesajlar ve masraf verileri" maddesi (DURUST: "uygulama uzerinden hicbir yonetici goremez, veritabani seviyesinde RLS ile kisitli" — 'kimse goremez' DEME). (3) Sonra: cihaz testi (Android dev build fee3ce6e kurulu mu?), Apple Developer capability + Supabase Apple provider (Ayse), Google marka dogrulamasi sonucu (e-posta), DM basliginda TUREB rozeti, docs/index.html eski palet, STORE-LISTING abonelik metni, sonra TEK store build 1.2.0 (`v1.2.0-yayin-komutlari.md`). Bugunku bloklar asagida (en yeni ustte degil — tarih etiketli).
>
> **[4 Eyl 2026 ogleden sonra] MADDE 6 — GIZLILIK (kodlandi + Supabase CANLI, commit yok).** (a) DM gorselleri **ozel bucket `dm-gorseller`** (migration `dm_gorselleri_ozel_bucket`; policy katilimci okur/yukler, gonderen siler, admin YOK); `dm_mesajlar.gorsel_url` = yol; istemci `dmGorselYukle`/`dmGorselUrl`/`DmMesajGorseli` (`components/sohbet-gorsel.tsx`), `app/dm/[id].tsx` gecirildi (DB'de 0 DM mesaji vardi, tasima gerekmedi). (b) UI cumleleri: DM baslik + bos ekran, DM listesi bos durumu, ajanda dipnotu, tur ekrani alt notu, profil "Gizlilik Politikasi" alt metni. (c) `app/gizlilik-politikasi.tsx` + `docs/index.html` (magaza Privacy Policy URL) yeniden yazildi — guncel veri envanteri, Supabase Frankfurt/AB, Expo/Resend/Google/Apple/TUREB, yeni Madde 5 "Ozel Mesajlar, Ajanda ve Masraf Verileri" (durust: yonetici uygulama uzerinden goremez, RLS, uctan uca degil, DB sahibinin teknik erisimi var); `kullanim-kosullari.tsx` 6 + 6a; `STORE-LISTING-BILGILERI.md` ucretsiz model + yeni ozellikler (STATE'teki "magaza abonelik metni" bekleyeni de kapandi). Tarihler 4 Eylul 2026. DECISIONS #57. tsc 0; eslint: yalnizca kullanim-kosullari.tsx'te 7 ESKI tirnak hatasi (bana ait degil). (d) Uctan uca sifreleme Ayse'ye sorulmadi/yapilmadi. **YARIN LISTESI 6/6 BITTI.** Sirada: cihaz testi (yeni dev build: image-manipulator + splash), commit (Ayse onayi), Apple Developer/Supabase Apple provider (Ayse), Google marka dogrulamasi sonucu, docs/index.html eski palet (ayri is), TEK store build 1.2.0.
>
> **[4 Eyl 2026 ogleden sonra] ACILIS EKRANI (SPLASH) YENIDEN — "eskisi gibi sik, gradyan sart degil" (Ayse).** Tani (prebuild-config kaynagi): Expo, Android 12+ icin logoyu `backgroundColor` dolgulu 288dp kanvasa basar, sistem bunu ikon sekliyle (Samsung squircle) maskeler → kanvas rengi pencere zemininden farkliysa "yuvarlak icinde logo" gorunur; Ayse'nin telefonu karanlik modda: zemin `dark` #0F1530, kanvas kobalt → cirkin kare; yazi 2/3 daire maskesi disina tastigi icin kesildi. Eski v1.1.1'de kare gorunmuyordu cunku iki renk de #005A8D idi. **Fix:** app.json splash: acik/koyu zemin AYNI kobalt #1E40AF (kare zemine karisir); Android gorseli `assets/images/splash-icon-android.png` (1024 kanvas, yalnizca pusula 520px = guvenli daire ici; imageWidth 240 → ekranda ~100dp logo); iOS gorseli `assets/images/splash-logo.png` (splash-icon.png'den kirpilmis pusula, imageWidth 100). Yazi native'de YOK (Android'de imkansiz). **Uygulama ici devam ekrani** `components/acilis-ekrani.tsx` (`AcilisEkrani gorunur`): DUZ kobalt zemin (gradyan kaldirildi), pusula native'deki yerinde (~100dp, ekran merkezi) baslayip 150dp'ye buyur, altinda `assets/images/splash-yazi.png` ("PUSULA ISTANBUL", splash-icon.png'den kirpildi) belirir; hazir olunca (en az 700 ms) 380 ms solar, unmount. `app/_layout.tsx`: `SplashScreen.hideAsync()` MOUNT'ta, `hazir = fontsLoaded && oturum !== null` → Stack ancak hazirsa mount (eski gating korunur). JS kismi Metro reload ile gorulur; native kisim (kare/yazi kirpma) YENI BUILD ister. Eski `splash-icon.png` (yazili) artik yalnizca referans; sloganli hali kullanilmiyor. NOT: yerel `android/`+`ios/` klasorleri gitignore'da ve BAYAT (27 Haz) — EAS bulutta prebuild yapar; `npx expo run:android` oncesi `npx expo prebuild --clean` sart.
>
> **AYRICA: NATIVE MODUL IMPORT DERSI.** `lib/avatar.ts` ustunde `expo-image-manipulator` import edilince eski dev build'de (modul yok) Sohbet/Ilanlar/Profil sekmeleri KAYBOLDU (route modulu yuklenirken throw → expo-router sekmeyi dusurdu). Fix: modul `manipulatorGetir()` icinde ihtiyac aninda `require` edilir, yoksa kucultme atlanir; onbellek `hooks/use-avatarlar.ts`'e tasindi (native bagimliliksiz). Kural: yeni native modulu ekran/hook seviyesinde import ETME, kullanildigi fonksiyonda lazy require et. Ayse cihazda dogruladi: profil fotografi calisiyor (eski build'de kucultmesiz).
>
> **[4 Eyl 2026 ogleden sonra] MADDE 5 — PROFIL FOTOGRAFI + AVATAR (kodlandi + Supabase CANLI, commit yok, cihaz testi yok).** Istege bagli. Migration **`profil_fotografi_avatar` UYGULANDI**: `profiles.avatar_url text`, bucket **`profil-fotolari`** (public, 2 MB, jpeg/png/webp; policy: herkes okur, sahibi `<uid>/` altina yukler/gunceller, siler; admin/moderator de silebilir). Kod: `lib/avatar.ts` (avatarSec Kamera/Galeri + allowsEditing kare, avatarIsle = ortadan kare kirp + 512px + JPEG 0.85 — **expo-image-manipulator ~14.0.8 YENI NATIVE MODUL** (`npx expo install`, plugin gerekmez; dev build fee3ce6e'de YOK → yeni dev build sart, store build zaten planli), avatarYukle = `<uid>/avatar.jpg` upsert + `profiles.avatar_url` = public URL + `?v=zaman` (onbellek kirici), avatarKaldir; oturum onbellegi `avatarOnbellek` + `avatarDinle`), `hooks/use-avatarlar.ts` (`useAvatarlar(ids)` toplu okuma, useTurebRozetleri deseni; `useAvatar(id)`), `components/avatar.tsx` (`Avatar` — url varsa expo-image yuvarlak, yoksa harf; `avatarRenk/avatarHarf`). Yerler: profil header (dokununca sec) + Profili Duzenle modalinda "Profil Fotografi" satiri (Fotograf Sec/Degistir · Kaldir; secince HEMEN yuklenir, isim sinirindan bagimsiz); genel sohbet balonlari + DM listesi (`sohbet.tsx`, `karsi_id`); DM basligi + bos ekran (`dm/[id].tsx`, `karsiId` yoksa mesajlardan); ilan karti sahip satiri 22px (`ilanlar.tsx`); tepki verenler listesi 24px (`TepkiOzeti.idler` eklendi). Foto yoksa eski harf avatari + renk mantigi birebir. tsc 0, eslint 0 hata (3 eski uyari). Cihaz testi bekliyor. Sirada: **6. madde gizlilik** (asagida).
>
> **[4 Eyl 2026 sabah] COMMIT + PUSH YAPILDI.** Dunku 97 dosya `d3fa5f9` (v1.2.0 hazirlik), sonra `b03fe09` (app.json 1.2.0 / iOS build 43 / Android versionCode 44 + `v1.2.0-yayin-komutlari.md`); push `f67f459..b03fe09`. GitHub push protection STATE.md'deki Resend API key'ini yakaladi → push edilmemis 4 commit filter-branch ile yeniden yazildi (anahtar redakte), **anahtar Haziran'dan beri origin gecmisinde de vardi → Ayse Resend'de "manuel-bilgilendirme" key'ini YENILEMELI (.env guncelle)**. `aboneler-premium.xlsx` bilerek commit disi. **MAGAZA BUILD'I YAPILMADI, ERTELENDI** — Ayse: "yapmak istedigim degisiklikler bitmedi" (Google/Apple girisi yeni native modul getirecek, tek store build sonda). Iptal edilen production build'ler app.json'u 44/45'e cekti (calisma kopyasinda, zararsiz). **Dev build alindi:** `eas build --platform android --profile development` (fee3ce6e) → S22'ye kurulacak, sonra `npx expo start --dev-client`. NOT: device_bash'te bazi dosyalar "Permission denied" veriyor (Metro/VS Code tutuyor) → dosyayi /tmp'ye yazip `mv -f` ile ustune tasi calisiyor; commit sonrasi `.git/*.lock` + `tmp_obj_*` kalintisi olursa silme izni al.
>
> **[4 Eyl 2026] YARIN LISTESI MADDE 1 — TELEFON: BEYAN USULU, SMS/WHATSAPP DOGRULAMA YOK (kodlandi, commit yok).** Maliyet arastirmasi yapildi (Netgsm OTP 1.000 SMS 316 TL, sahis basligi icin kimlik+ikametgah; Twilio TR'de evraksiz filtrelenir; Firebase Blaze sart; WhatsApp Cloud API ~$0,005–0,01/mesaj ama WhatsApp'a kayitsiz ayri numara ister) → Ayse: "yeni kart almak istemiyorum", karar **dogrulama kodu YOK, kayitta telefon ZORUNLU**. Yapilan: `lib/telefon.ts` (E.164 normalize +90 varsayilan, yurt disi + / 00 kabul, `telefonGoster`, `whatsappLink`, `aramaLink`); `components/telefon-modal.tsx` (`TelefonAlani` = input + bicim uyarisi + "Numarami WhatsApp'ta ac ve kontrol et" kendi kendine saglama; `TelefonModal`; `useTelefonGerekli(islem)` — DM baslatmadan once telefon yoksa modal, kaydedince islem devam); `components/telefon-karti.tsx` ana sayfada (GuncellemeBandi altinda) telefonu bos mevcut kullanicilara tek seferlik kart, "Sonra" 3 gun erteler (AsyncStorage `telefon-karti-ertele`); `app/giris.tsx` kayit formuna zorunlu "Cep telefonu" (metadata `telefon`, profil upsert/insert'e eklendi); **migration `handle_new_user_telefon` UYGULANDI** (trigger metadata telefonunu profile yazar, ON CONFLICT'te bos telefonu doldurur); `hooks/use-ilanlar.ts` `telefonKaydet` normalize eder (gecersiz → false) + `telefonNumaraRaporla(ilan)` → `raporlanan_mesajlar` kaynak='telefon', sebep='diger', mesaj_id=ilan id (yetkili SohbetYonetim Raporlar'da "[Telefon bildirimi]" olarak gorur; "Sil" sohbet mesaji aramaz, no-op); `app/(tabs)/ilanlar.tsx` ilan formunda telefon bicim kontrolu (E.164 saklanir), baskasinin ilaninda "Numaraya ulasilamiyor mu? Bildir" linki, `onMesaj` telefonGerekli ile sarili; `app/(tabs)/sohbet.tsx` `dmBaslat` telefonGerekli ile sarili; `app/(tabs)/profil.tsx` header'da telefon (bos ise "Telefon eklenmedi"), duzenlemede TelefonAlani + kaydette zorunlu/normalize. tsc 0, eslint yeni dosyalarda 0. **Cihaz testi bekliyor.** Sirada: madde 2 (Google + Apple giris), 3 (TUREB kontrolu — karar sorulacak), 4 (dil listesi), 5 (profil fotografi), 6 (gizlilik metinleri + DM gorselleri ozel bucket).
>
> **[4 Eyl 2026] MADDE 2 — GOOGLE + APPLE ILE GIRIS (kodlandi, commit yok, cihaz testi yok).** Google Cloud: proje `pusula-istanbul` (Firebase'inki), Google Auth Platform → Branding (ad, destek maili, ana sayfa `https://pusulaistanbul.app`, gizlilik `https://pusulaistanbul.app/#yasal`, Authorised domain `pusulaistanbul.app`; logo YOK — logo yuklenirse verification ister), Audience → In production, Clients → "Supabase Auth" Web istemcisi (redirect `https://rzlfghjpsximthlolfxo.supabase.co/auth/v1/callback`; Client ID `56489756536-leln4qs6...apps.googleusercontent.com`, secret Ayse'nin Downloads JSON'unda). Supabase → Providers → Google ENABLE (ID+secret girildi); Redirect URLs'e `http://localhost:8081/**` eklendi (web test). Teknik tercih: Google = tarayici akisi (`signInWithOAuth` skipBrowserRedirect + `expo-web-browser.openAuthSessionAsync`, donus `pusulaistanbul://giris`, implicit #access_token → setSession / PKCE ?code → exchangeCodeForSession; web'de sayfa yonlenir, giris.tsx mount'ta `oauthDonusuIsle(window.location.href)`); Apple = yalnizca iOS yerli (`expo-apple-authentication` ~8.0.8 yuklendi, `signInWithIdToken`; ad/soyad yalnizca ilk giriste gelir → profile yazilir); Android'de Apple girisi SUNULMAZ (Supabase Apple secret 6 ayda bir yenilenmesin). Kod: `lib/oauth.ts` (googleIleGiris, appleIleGiris, appleGirisiVarMi, oauthDonusuIsle, profilEksikMi), `app/giris.tsx` (ayrac + "Google ile devam et" + iOS'ta "Apple ile devam et", Google logosu HEX istisnasi), `app/profil-tamamla.tsx` (OAuth ile gelen: isim/soyisim metadata'dan, TUREB ruhsat no + telefon zorunlu, upsert + updateUser → USER_UPDATED → _layout yeniden kontrol; Cikis yap), `app/_layout.tsx` (`profilEksik` state; SIGNED_IN/INITIAL_SESSION/USER_UPDATED'da `profilEksikMi`; eksikse her yerden /profil-tamamla'ya, yonlendirmeden once taze kontrol = dongu sigortasi; giris→tabs yonlendirmesi profil kontrolu bitmeden beklemez... bekler: `profilEksik === false` sarti), migration **`handle_new_user_oauth_metadata` UYGULANDI** (given_name/family_name/full_name/name fallback; kuru test OK), app.json `ios.usesAppleSignIn: true` + plugin `expo-apple-authentication` (→ native, store build'e girer). Kural: `profilEksikMi` yalnizca ruhsat_no/isim bos ise eksik der (telefonu bos ESKI kullanicilar TelefonKarti ile yumusak). NOT: profiles'ta ruhsat_no bos 2 hesap var (Ayse admin `ayse.tokkus@gmail.com` "Pusula", Orcun moderator) → bir kez profil-tamamla gorurler. **AYSE'NIN KALAN ADIMLARI:** (1) Apple Developer → Identifiers → `com.pusulaistanbul.app` → Sign in with Apple capability isaretle → Save (sonraki `eas build` provisioning'i yeniler); (2) Supabase → Providers → Apple → Enable, Client IDs: `com.pusulaistanbul.app` (secret gerekmez, native akis); (3) Chrome'da Google girisini test et (`npx expo start` → w); Android dev build'de Google (sistem tarayici → geri donus); Apple icin iOS dev build. tsc 0, eslint 0.
>
> **[4 Eyl 2026] GOOGLE MARKA DOGRULAMASI BASVURULDU (bekliyor).** Consent ekraninda "Pusula Istanbul" yerine `rzlf...supabase.co` gorunuyordu (dogrulanmamis uygulama → redirect alan adi). Secenek 1 secildi (ucretsiz marka dogrulamasi; Supabase custom domain eklentisi REDDEDILDI). Yapilan: Search Console'da `pusulaistanbul.app` Domain mulku (GoDaddy baglantisiyla TXT `google-site-verification=RpL1...0AQ` eklendi, dogrulandi); Branding: logo ilk basvuruda "jenerik, markayi tanimlamiyor" diye reddedildi → yazili logo `assets/images/google-oauth-logo-512.png` (pusula gulu + PUSULA ISTANBUL, Pillow ile uretildi) yuklendi, yeniden basvuru "I have fixed the issues". Sonuc Google'dan e-postayla gelir (2–5 is gunu, ayse.tokkus@gmail.com). Reddederse: docs/index.html logosunu da ayni gorsele cevir + `docs/gizlilik.html` ayri sayfa (bot `#yasal`'i ana sayfa sayabilir). Google client secret YENILENDI (4 Eyl ~11:00: sohbette gorunen `…G9ou` DISABLE edildi, yeni secret Supabase'de; `invalid_client` hatasinin sebebi kopyalama hatasiydi — kontrol yontemi: Supabase'e yapistirilanin son 4 karakteri Google listesindekiyle ayni olmali). Resend key'i de yenilendi. Android dev build'de Google akisi acildi (Custom Tab), giris basarili (Chrome web'de de).
>
> **[4 Eyl 2026] MADDE 4 — DIL LISTESI:** `constants/diller.ts` = TUREB "Rehberler Dillere Gore" xls'indeki 39 dil (eylemli sayisina gore sirali) + en sonda **Turkce** (Turk grup ilanlari; Ayse). Yazim her yerde **TUREB** (U noktasiz).
>
> **[4 Eyl 2026] MADDE 3 — TUREB DOGRULAMASI (kodlandi + Supabase CANLI, commit yok).** TUREB `https://www.tureb.org.tr/RehberVeritabani` acik sorgu: `POST /RehberVeritabani/AraAdiSoyadi` (adi, soyadi; captcha yok) → HTML tablo: Adi Soyadi · Oda · Yabanci Dil · Telefon · E-posta · Eylemli/Eylemsiz. **RUHSAT NO YAYINLANMIYOR** → dogrulama = ad-soyad eslesmesi. Ayse kararlari: rozet modeli, kayit ENGELLENMEZ; eylemsiz rehber de ilan verir/basvurur. **Supabase (uygulandi):** migration `tureb_dogrulama` (profiles.tureb_durum eylemli|eylemsiz|bulunamadi|coklu|bilinmiyor, tureb_oda, tureb_dil, tureb_ad, tureb_adaylar jsonb, tureb_kontrol_at; BEFORE UPDATE trigger `profiles_tureb_koru` — service_role/admin/dogrudan SQL disinda tureb_* degisiklikleri OLD'a geri alinir; fix migration `profiles_tureb_koru_role_fix`: rol `request.jwt.claims`->>'role' ile okunur), migration `tureb_http_pg_net_rpc` + `tureb_http_baslat_header_fix` (SECURITY DEFINER `tureb_http_baslat(qs)` → net.http_post query-string ile, `tureb_http_sonuc(rid)`; yalnizca service_role EXECUTE; ozel User-Agent IIS'te 400 verdi → kaldirildi). **Edge Function `tureb-dogrula` v3 CANLI** (verify_jwt): Deno fetch TUREB'e baglanamiyor ("connection reset", TLS 1.2 sunucu) → cagri pg_net uzerinden (RPC baslat + 400ms yoklama, ~1.3 sn); HTML entity cozumu (&#231; → c); cift soyad/ikinci ad icin 4 asamali sorgu; tam ad eslesme → tek: eylemli/eylemsiz + oda/dil/ad (+ profiles.diller bossa TUREB dili, virgulle ayrilir); coklu → adaylar (max 20) kullanici secer (`{secim:n}`); yok → bulunamadi; erisim yok → bilinmiyor (mevcut sonuc bozulmaz). E2E test hotmail test hesabiyla: tek eslesme (AYSE TOKKUS BAYAR · IRO · Fransizca · Eylemli), coklu (MEHMET DEMIR ×2 → secim), Turkce karakter (ALI OZTURK · Arapca) OK; kullanicinin kendi tureb_* yazma denemesi geri alindi (test sonrasi hesap sifirlandi). **Kod:** `hooks/use-tureb.ts` (turebDogrula, useTureb, useTurebOtomatik — acilista oturum basina 1 kez: hic kontrol yok / 180 gun eski / ad degisti / bilinmiyor → yeniden; useTurebRozetleri — id listesi icin toplu okuma + onbellek), `components/tureb-rozet.tsx` (TurebRozet "TUREB · IRO" yesil / "· Eylemsiz" gri; TurebKarti profil ekrani: durum aciklamasi, "Kaydimi sec" aday modali, "Yeniden kontrol et"), `components/yetkili/tureb-yonetim.tsx` (admin: eslesmeyen/eylemsiz/eylemli listeleri + sayilar; profil ekraninda YetkiliBolum "TUREB Eslesmeleri"), profil.tsx header'da rozet + TurebKarti, ilanlar.tsx kartta sahibin rozeti, _layout.tsx `useTurebOtomatik(oturum && profilEksik===false)`. tsc 0, eslint 0. DM basliginda rozet YOK (sonra). Cihaz testi bekliyor.
>
> ### Onceki blok (3 Eylul 2026, gece — oturum kapanisi)
>
...
> 4. **Rehber ilanindaki dil listesinin guncellenmesi** (`constants/diller.ts`, 40 dil (TUREB listesi 39 + Turkce en sonda, 4 Eyl); Ayse'den guncel liste/degisiklik alinacak — TUREB dil listesiyle karsilastir).
...
> 7. "Rehber Araniyor" ilan sekmesi (Ara sekmesinin yerine; migration `ilanlar_ve_rotalar`, push-gonder v6 dil filtreli, takvim, TUREB taban kontrolu, 40 dil (TUREB listesi 39 + Turkce en sonda, 4 Eyl), "Bildirim dillerim").
